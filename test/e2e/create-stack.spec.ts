...
test.describe("создание стека на отдельной странице", () => {
    ...
    test("команда docker run превращается в compose и стек появляется в списке", async ({ page }) => {
        ...
        await form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i }).fill(`docker run -d --name ${NEW_STACK} -p 8099:80 --rm traefik/whoami`);

        // Через задержку распознавания команда становится Compose в том же поле.
        await expect(form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i })).toHaveValue(/services:/, { timeout: 5000 });
        await expect(form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i })).toHaveValue(new RegExp(`container_name: ${NEW_STACK}`));
    ...
    test("исходную команду можно вернуть, и поле остается своим", async ({ page }) => {
        ...
        await form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i }).fill(command);
        await expect(form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i })).toHaveValue(/services:/, { timeout: 5000 });
        await form.getByRole("button", { name: /вернуть команду|bring the command back/i }).click();

        await expect(form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i })).toHaveValue(command);
    ...
    test("готовый compose не изменяется, ошибка проверки сохраняет форму", async ({ page }) => {
        ...
        await form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i }).fill(broken);
    ...
    test("вставка с экрана стека открывает заполненную вкладку Compose", async ({ page }) => {
        ...
        const form = page.getByRole("tabpanel", { name: /вставить compose|paste compose/i });
        await expect(form).toBeVisible();
        await expect(form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i })).toBeFocused();
        await expect(form.getByRole("textbox", { name: /^(что развернуть|what to deploy)$/i })).toHaveValue(/services:/, { timeout: 5000 });
