...
test("окно без наблюдений не становится зелёным", () => {
...
test("пропуск в наблюдениях уменьшает покрытие, а не создаёт зелёное время", () => {
    ...
    assert.equal(result.coveredMs, DAY);
    // Шесть часов «неизвестно» здоровыми не считаются
    assert.ok((result.ratio ?? 1) < 0.8, String(result.ratio));
    assert.equal(result.incidents, 1);
});
