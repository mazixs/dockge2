...
- Рассмотреть переход с самописного JWT/TOTP на Better Auth.
- Better Auth использовать для пользователей, паролей, сессий, cookies, 2FA и базовой защиты auth-границы.
...
- Переход на Better Auth признан целесообразным, но отдельной миграцией, а не смешанным изменением зависимостей и Stack.
- Better Auth отвечает за authentication/session/2FA; authorization capabilities, агентскую аутентификацию и registry secrets проектируются отдельно.
