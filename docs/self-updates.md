...
installation. The `0.0.12` release adds a tested import contract for this exact condition. It
...
the published repository, use the released `0.0.12` bootstrap, always naming the directory:

```bash
curl --proto '=https' --proto-redir '=https' -fsSL \
  https://github.com/mazixs/dockge2/releases/download/v0.0.12/install.sh \
  -o /tmp/dockge2-install-0.0.12.sh
sudo bash /tmp/dockge2-install-0.0.12.sh --update --dir /opt/dockge2 --version 0.0.12 --dry-run
sudo bash /tmp/dockge2-install-0.0.12.sh --update --dir /opt/dockge2 --version 0.0.12 --yes
```

An already managed `0.0.10` installation instead uses its installed `.dockge2/update` executable
with `--dir /opt/dockge2 --update --version 0.0.12` and first checks `--dry-run`. Both paths keep
